...
fromArray :: forall s a. Nat s => s -> Array a -> Maybe (Vec s a)
fromArray = const fromArray'

fromArray' :: forall s a. Nat s => Array a -> Maybe (Vec s a)
fromArray' xs =
  let i = toInt (undefined :: s)
  in  if Array.length xs < i then Nothing else Just $ Vec $ Array.take i xs
